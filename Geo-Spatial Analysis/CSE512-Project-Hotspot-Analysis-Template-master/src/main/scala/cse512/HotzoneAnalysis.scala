...
object HotzoneAnalysis {
  ...
  //path to point path to rectangle
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    // Grouping rectangles and sorting ascending order
    val groupdf = joinDf.groupBy("rectangle").agg(count("rectangle").alias("Count"))
    groupdf.sort(asc("rectangle"))
    return groupdf.coalesce(1) // YOU NEED TO CHANGE THIS PART
  }
